...
import { config, timeline, draw } from '../assets/js/showcase.js';
import metadata from '../assets/js/data/showcase-exports.js';
...
for(const [id,preset] of Object.entries(config.presets)) {
 ...
 const frames=gif.frames.filter(f=>f.image),duration=meta.duration;
 ...
 assert.equal(frames.length,Math.round(duration*meta.fps));
 ...
 assert.deepEqual(scenes.filter(s=>s.page).map(s=>s.page),preset.pages);
