...
function __OriginkitBase_DeckCarousel({
    ...
    useEffect(() => {
        ...
        const onStep = (event: Event) => {
            ...
            const cardsPerLap = frame.current.lapPx / frame.current.step
            state.progressTarget =
                (Math.round(state.progressTarget * cardsPerLap) + direction) / cardsPerLap
        }
        ...
        const onTouchStart = (event: TouchEvent) => {
            ...
            touchPoint = state.ready && touch ? { x: touch.clientX, y: touch.clientY } : null
        }
        const onTouchMove = (event: TouchEvent) => {
            const touch = event.touches[0]
            if (!touch || !touchPoint) return
            const dx = touchPoint.x - touch.clientX
            const dy = touchPoint.y - touch.clientY
            const delta = clamp(
                (Math.abs(dx) > Math.abs(dy) ? dx : dy) * TOUCH_GAIN,
                -WHEEL_CLAMP,
                WHEEL_CLAMP
            )
        ...
        node.addEventListener("deck-step", onStep)
        ...
        return () => {
            ...
            node.removeEventListener("deck-step", onStep)
...
export default function DeckCarousel(props: Record<string, unknown>) {
  return <__OriginkitBase_DeckCarousel {...(__originkitPresetProps as Record<string, unknown>)} {...props} />;
}
